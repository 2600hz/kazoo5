...
When starting new work, create a branch off the relevant parent branch (like `master`, `4.3`, etc). The branch should be named based on the ticketing / support system tracking the work (JIRA board, support system ticket ID, etc) such as `TICKET-6120`:

    git checkout -b TICKET-6120 origin/master
...
3. Now that you have a nice commit message and the code is complete (as far as you know), you are ready to push the branch up and open a pull request: `git push origin TICKET-6120`

An example commit message:

```
TICKET-6120: update documentation to establish guidance on git branches and commits

This change provides the reader with some example git commands for managing branches and commits within the {% BRAND_NAME %} project. The goal of this document is to help contributors format their pull request commit to better serve future readers of the code.
...
Finally, the doc will include examples of how {% BRAND_NAME %} committers can pull down a pull request and merge it to the parent branch locally, effectively emulating Github's squash-and-merge functionality.
```
...
e15fb20c63 * TICKET-6120: document a bit about git and commit hygiene
```
...
1. Fetch the PR branch: `git checkout -b TICKET-6120 origin/TICKET-6120`
...
4. Edit the commit message to append the Github pull request ID to the commit subject:
   `TICKET-6120: document a bit about git and commit hygiene` => `TICKET-6120: document a bit about git and commit hygiene (#9988)`
5. Push the modified branch back up to Github: `git push -v --force-with-lease origin TICKET-6120:refs/heads/TICKET-6120`
...
8. Last step is to delete the local and remote feature branches:
   ```
   git branch -d TICKET-6120
   git push origin :TICKET-6120
   ```
